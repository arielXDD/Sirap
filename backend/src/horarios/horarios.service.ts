...
export class HorariosService {
  ...
  async bulkUpdate(params: { deleteIds: number[], upsertData: any[] }): Promise<void> {
    await this.horarioRepository.manager.transaction(async transactionalEntityManager => {
      // 1. Borrar IDs
      if (params.deleteIds.length > 0) {
        await transactionalEntityManager.delete(Horario, params.deleteIds);
      }
      // 2. Upsert (Actualizar o Insertar)
      for (const data of params.upsertData) {
        if (data.id) {
          await transactionalEntityManager.update(Horario, data.id, data);
        } else {
          const newH = transactionalEntityManager.create(Horario, data);
          await transactionalEntityManager.save(Horario, newH);
        }
      }
    });
  }
