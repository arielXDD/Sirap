...
export class AsistenciasService {
  ...
  async registrarAsistencia(
    ...
    const now = new Date();
    const diaSemana = this.obtenerDiaSemana(now);
    const horaActualStr = now.toTimeString().split(' ')[0]; // HH:MM:SS
    const [hNow, mNow] = horaActualStr.split(':').map(Number);
    const minutosNow = hNow * 60 + mNow;

    // Buscar si ya hay una asistencia abierta para hoy
    ...
    if (!asistencia) {
      // REGISTRAR ENTRADA - Validar si tiene turno vigente AHORA
      const horarios = await this.horarioRepository.find({
        where: { empleadoId, diaSemana, activo: true }
      });

      if (horarios.length === 0) {
        throw new BadRequestException('Usted no cuenta con turno asignado para el día de hoy.');
      }

      // Encontrar si hay un turno cuyo rango [Entrada - 3h, Salida] incluya el momento actual
      // 3 horas antes es el margen para que puedan entrar temprano
      const horarioVigente = horarios.find(h => {
        const [hEnt, mEnt] = h.horaEntrada.split(':').map(Number);
        const [hSal, mSal] = h.horaSalida.split(':').map(Number);
        const minEnt = hEnt * 60 + mEnt;
        const minSal = hSal * 60 + mSal;
        
        // El margen para entrar es desde 3 horas antes hasta el final de la jornada (o el fin del turno)
        return minutosNow >= (minEnt - 180) && minutosNow <= minSal;
      });

      if (!horarioVigente) {
        throw new BadRequestException('Usted no tiene un turno activo asignado en este horario.');
      }
      ...
      // Calcular estado comparando con el "horarioVigente" encontrado
      const [hEsp, mEsp] = horarioVigente.horaEntrada.split(':').map(Number);
      const minEsp = hEsp * 60 + mEsp;
      const retard = minutosNow - minEsp;
      ...
      if (retard <= 0) {
        ...
        asistencia.minutosRetardo = retard;
      }

      return await this.asistenciaRepository.save(asistencia);
    } else {
      // REGISTRAR SALIDA
      asistencia.horaSalida = horaActualStr;
      return await this.asistenciaRepository.save(asistencia);
    }
  }
  ...
  /**
   * Obtiene el día de la semana en español de forma segura (sin desfase horario)
   */
  private obtenerDiaSemana(fecha: any): string {
    const dias = [
      'domingo',
      'lunes',
      'martes',
    ...
    // Si es un objeto Date, lo normalizamos para evitar desfases por zona horaria
    const dateObj = typeof fecha === 'string' ? new Date(fecha + 'T12:00:00') : new Date(fecha);
    
    // Si la fecha ya es un objeto Date de TypeORM (que suele venir a las 00:00:00), 
    // forzamos el mediodía para que getDay() sea el correcto independientemente de la zona horaria local.
    const normalized = new Date(dateObj.getFullYear(), dateObj.getMonth(), dateObj.getDate(), 12, 0, 0);

    return dias[normalized.getDay()];
  }
  ...
  async findByEmpleadoYFechas(
    ...
    return await this.asistenciaRepository.find({
      ...
      relations: ['empleado', 'empleado.horarios'],
      order: { fecha: 'DESC' },
    });
  }
  ...
  /**
   * Calcula automáticamente si la asistencia es puntual o retardo (usado para registros manuales)
   */
  private async calcularEstadoAsistencia(
    asistencia: Asistencia,
    empleado: Empleado,
  ): Promise<void> {
    const diaSemana = this.obtenerDiaSemana(asistencia.fecha);
    const [hNow, mNow] = asistencia.horaEntrada.split(':').map(Number);
    const minutosNow = hNow * 60 + mNow;

    const horarios = await this.horarioRepository.find({
      where: { empleadoId: empleado.id, diaSemana, activo: true }
    });
    ...
    let horarioVigente = horarios[0];
    let diffMin = Infinity;
    horarios.forEach(h => {
        const [hE, mE] = h.horaEntrada.split(':').map(Number);
        const diff = Math.abs(minutosNow - (hE * 60 + mE));
        if (diff < diffMin) {
            diffMin = diff;
            horarioVigente = h;
        }
