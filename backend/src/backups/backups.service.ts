...
export class BackupsService {
  ...
  async remove(id: number) {
    ...
    try {
      // Intenta eliminar el archivo físico de forma asíncrona para no bloquear el hilo principal
      if (fs.existsSync(backup.ruta)) {
        await fs.promises.unlink(backup.ruta);
      }
    } catch (error) {
      console.error(`Error al eliminar archivo físico: ${error.message}`);
      // Continuamos eliminando el registro de la DB aunque el archivo falle
    }

    return await this.backupRepository.delete(id);
  }
}
